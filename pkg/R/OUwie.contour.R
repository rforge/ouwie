...
OUwie.contour<-function(phy,data, model=c("BM1","BMS","OU1","OUM","OUMV","OUMA","OUMVA"), simmap.tree=FALSE, root.station=TRUE, lb=0.000001, ub=1000, focal.param=NULL, clade=NULL, mserr=FALSE, nrep=1000, sd.mult=3, levels=c(0.5,1,1.5,2),likelihood.boundary=Inf,lwd=2, ...){
#focal.param is something like c("alpha_2","sigma.sq_1"). They are then split on "_"
	...
	globalMLE<-OUwie(phy=phy,data=data,model=model,simmap.tree=simmap.tree,root.station=root.station,lb=lb, ub=ub, clade=clade, mserr=mserr)
	...
	dev<-function(p,phy,data,model,simmap.tree,root.station,focal.param.vector,clade,mserr,globalMLE){ 
	#globalMLE is just for figuring out the structure of alpha and sigma.sq
		...
		loglik<-OUwie.fixed(phy=phy,data=data, model=model,simmap.tree=simmap.tree,root.station=root.station,alpha=alpha, sigma.sq=sigma.sq, theta=NULL, clade=clade, mserr=mserr)$loglik
		if(loglik==Inf){
			loglik=-10000000
		}
	...
	optimizeSemifixed<-function(X,phy,data,model,simmap.tree,root.station,lb,ub,clade,mserr,globalMLE) {
		...
		out = nloptr(x0=rep(ip, length.out = np), eval_f=dev, opts=opts, phy=phy,data=data, model=model,simmap.tree=simmap.tree,root.station=root.station, lb=lower, ub=upper, clade=clade, mserr=mserr, globalMLE=globalMLE,focal.param.vector=focal.param.vector)
	...
	likelihoods<-(sapply(params.points.list,optimizeSemifixed,phy=phy,data=data, model=model, simmap.tree=simmap.tree,root.station=root.station,lb=lb,ub=ub,clade=clade,mserr=mserr,globalMLE=globalMLE,simplify=TRUE))
